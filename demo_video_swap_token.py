...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_image_path', type=str, default='', help='Path to the input image')
    parser.add_argument('--input_videos_path', type=str, default='', help='Path to the input video')
    ...
    image_token = cv2.imread(args.input_image_path)
    if args.crop:
        kpt_mediapipe = run_mediapipe(image_token)
        
        kpt_mediapipe = kpt_mediapipe[..., :2]

        tform = crop_face(image_token,kpt_mediapipe,scale=1.4,image_size=input_image_size)
        
        cropped_image = warp(image_token, tform.inverse, output_shape=(224, 224), preserve_range=True).astype(np.uint8)
        ...
        cropped_image = image_token
        cropped_kpt_mediapipe = image_token
            
    cropped_image = cv2.cvtColor(cropped_image, cv2.COLOR_BGR2RGB)
    cropped_image = cv2.resize(cropped_image, (224,224))
    cropped_image = torch.tensor(cropped_image).permute(2,0,1).unsqueeze(0).float()/255.0
    cropped_image_token = cropped_image.to(args.device)

    #the target token
    outputs = smirk_encoder(cropped_image_token)
    inv_token = outputs['token']
    

    for video_sub_path in os.listdir(args.input_videos_path):
        
        video_path = os.path.join(args.input_videos_path,video_sub_path)
        ...
        f_out = f"{args.out_path}/{video_path.split('/')[-1].split('.')[0]}.mp4"
        ...
        for _ in range(n_frames):
            ...
            if args.render_orig:
                ...
                grid = torch.cat([full_image, rendered_img_orig, (full_image + rendered_img_orig)/2], dim=3)
            else:
                grid = torch.cat([cropped_image_token, rendered_img], dim=3)
            ...
            if args.use_smirk_generator:
                ...
                reconstructed_img = smirk_generator(smirk_generator_input, inv_token)
                ...
                if args.render_orig:
                    ...
                    grid = torch.cat([grid, reconstructed_img_orig, cropped_image], dim=3)
                else:
                    grid = torch.cat([grid, reconstructed_img, cropped_image], dim=3)
