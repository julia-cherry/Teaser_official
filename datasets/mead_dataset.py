            ...
            if file in train_subjects:
                ...
                for root, dirs, files in os.walk(base_path_video):
                    if root.count(os.sep) - base_path_video.count(os.sep) == 0:
                        for dir in dirs:
                            #只要front目录下的
                            front_path = os.path.join(root, dir, 'front')
                            if os.path.exists(front_path) and os.path.isdir(front_path):
                                for subdir, _, _ in os.walk(front_path):
                                    for path in glob.glob(os.path.join(subdir, '*.mp4')):
                                        ...
                                        if os.path.exists(video_path) and os.path.exists(fan_landmarks_path) and os.path.exists(mediapipe_landmarks_path):
                                            train_list.append([video_path, fan_landmarks_path, mediapipe_landmarks_path, file])
                            ...
                            if os.path.exists(front_path) and os.path.isdir(front_path):
                                for subdir, _, _ in os.walk(front_path):
                                    for path in glob.glob(os.path.join(subdir, '*.mp4')):
                                        ...
                                        mediapipe_landmarks_path = path.replace(base_path_video,base_path_mediapipe_landmark).replace('.mp4','.npy')
                                        if os.path.exists(video_path) and os.path.exists(fan_landmarks_path) and os.path.exists(mediapipe_landmarks_path):
                                            val_list.append([video_path, fan_landmarks_path, mediapipe_landmarks_path, file])
                            ...
                            if os.path.exists(front_path) and os.path.isdir(front_path):
                                for subdir, _, _ in os.walk(front_path):
                                    for path in glob.glob(os.path.join(subdir, '*.mp4')):
                                        ...
                                        mediapipe_landmarks_path = path.replace(base_path_video,base_path_mediapipe_landmark).replace('.mp4','.npy')
                                        if os.path.exists(video_path) and os.path.exists(fan_landmarks_path) and os.path.exists(mediapipe_landmarks_path):
                                            test_list.append([video_path, fan_landmarks_path, mediapipe_landmarks_path, file])
