...
class SmirkTrainer(BaseTrainer):
    def __init__(self, config):
        ...
        lmk203_path = 'assets/203_landmark_embedding/203_landmark_embeding_new.npz'
    ...
    def step1(self, batch):
        ...
        lip_in_mp_indices = [72, 85, 71, 70, 69, 65, 87, 88, 89, 103, 90, 100, 98, 102, 95, 68, 77, 84, 80, 82, 73, 86, 74, 75, 76, 66, 94, 93, 92, 104, 91, 99, 97, 101, 96, 67, 78, 83, 79, 81]
        mp_lip_indices = torch.tensor(lip_in_mp_indices).to(self.config.device)
        
        kp_mouth_indices = [x for x in range(48, 61)]
        kp_mouth_indices = torch.tensor(kp_mouth_indices).to(self.config.device)
        
        mp_mouth_flame_list = []
        for i in range(len(mp_lip_indices)):
            mp_mouth_flame_list.append(flame_output['landmarks_mp'][:,mp_lip_indices[i]])
        mp_mouth_flame = torch.stack(mp_mouth_flame_list, dim=1) #40个，一共105个,（40，2）
        
        mp_mouth_gt_list = []
        for i in range(len(mp_lip_indices)):
            mp_mouth_gt_list.append(batch['landmarks_mp'][:,mp_lip_indices[i]])
        mp_mouth_gt = torch.stack(mp_mouth_gt_list, dim=1)
        
        losses['landmark_loss_mp_mouth'] = F.mse_loss(mp_mouth_flame, mp_mouth_gt)
        fan_mouth_flame = flame_output['landmarks_mp'][:,kp_mouth_indices]
        fan_mouth_gt = batch['landmarks_mp'][:,kp_mouth_indices]
        losses['landmark_loss_fan_mouth'] = F.mse_loss(fan_mouth_flame, fan_mouth_gt)
        ...
        if self.config.arch.enable_fuse_generator:
            ...
            reconstructed_encoder_output = self.smirk_encoder(reconstructed_img)
        ...
        landmark_losses = losses['landmark_loss_fan'] * self.config.train.loss_weights['landmark_loss'] + \
                            losses['landmark_loss_mp'] * self.config.train.loss_weights['landmark_loss'] + \
                                ...
                                losses['landmark_loss_fan_mouth']* self.config.train.loss_weights['landmark_loss_fan_mouth'] +\
                                    losses['landmark_loss_mp_mouth']* self.config.train.loss_weights['landmark_loss_mp_mouth']
        ...
        # print(len(ld_203_with_angle_gt))
    ...
    def step(self, batch, batch_idx, losses_AM_l1, losses_AM_per, losses_AM_token_cycle, losses_AM_203_landmark, losses_AM_landmark_mouth_fan, losses_AM_landmark_mouth_mp, phase='train'):
        ...
        losses_AM_landmark_mouth_fan.update(losses['landmark_loss_fan_mouth'])
        losses['landmark_loss_fan_mouth'] = losses_AM_landmark_mouth_fan.avg
        
        losses_AM_landmark_mouth_mp.update(losses['landmark_loss_mp_mouth'])
        losses['landmark_loss_mp_mouth'] = losses_AM_landmark_mouth_mp.avg
